...
def test_ensure_collection_creates():
    s = _store()
    s.ensure_collection("my_show__bge-m3", show="My Show", model="bge-m3", dim=1024)
    assert s.collection_exists("my_show__bge-m3")


def test_ensure_collection_idempotent():
    s = _store()
    s.ensure_collection("c", show="S", model="m", dim=8)
    s.ensure_collection("c", show="S", model="m", dim=8)  # no exception
...
def test_list_collections_all():
    s = _store()
    s.ensure_collection("b__m", show="B", model="m", dim=8)
    s.ensure_collection("a__m", show="A", model="m", dim=8)
    assert s.list_collections() == ["a__m", "b__m"]


def test_list_collections_filter_show():
    s = _store()
    s.ensure_collection("a__bge-m3", show="A", model="bge-m3", dim=1024)
    s.ensure_collection("b__bge-m3", show="B", model="bge-m3", dim=1024)
...
def test_list_collections_filter_model():
    s = _store()
    s.ensure_collection("a__bge-m3", show="A", model="bge-m3", dim=1024)
    s.ensure_collection("a__e5", show="A", model="e5", dim=384)
...
def test_load_chunks_embedding_shape():
    s = _store()
    s.ensure_collection("c", show="S", model="m", dim=16)
