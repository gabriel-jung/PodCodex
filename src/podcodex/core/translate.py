    ...
    for i, seg in enumerate(segments):
        ...
        if missing:
            raise ValueError(
                ...
                "Make sure you paste back the full JSON array, not a minimal LLM summary."
            )
...
def translation_to_text(
    segments: list[dict], lang: Literal["source", "trad", "both"] = "both"
) -> str:
    """
    ...
    Args:
        ...
        lang     : "source" (original), "trad" (translated), "both" (side by side)
    """
    lines = []
    for seg in segments:
...
def build_manual_prompt(
    ...
    if task == "polish":
        ...
        instruction = (
            f"Correct and translate the {len(segments)} segments below from {source_lang} to {target_lang}.\n"
            "Return the SAME JSON array with the 'text' field corrected and a 'text_trad' field added to each entry. "
            "Keep all other fields (speaker, start, end, etc.) exactly as-is. "
