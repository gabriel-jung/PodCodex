...
Fix
---
podcodex never uses torch.compile / torch._dynamo for inference, so we
...
_DIAG_PATH = os.path.join(tempfile.gettempdir(), "podcodex_rt_hook.log")
...
class _TransformersStubFinder:
    ...
    2. transformers.generation.candidate_generator
       ...
       etc.) are speculative-decoding helpers podcodex's TTS engines do not
    ...
    def find_spec(self, fullname, path=None, target=None):
        ...
        for finder in sys.meta_path:
            ...
            try:
                ...
                _diag(
                    f"[scipy-finder] inner finder {type(finder).__name__} raised: {e}"
                )
                continue
            ...
            if real_spec.loader is None:
                _diag(
                    f"[scipy-finder] {type(finder).__name__} returned spec with loader=None"
                )
                continue
            _diag(
